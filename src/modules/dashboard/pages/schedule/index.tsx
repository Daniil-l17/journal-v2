...
export const Schedule = () => {
	...
	return (
		<div className='flex min-h-0 w-full min-w-0 flex-col gap-4 max-lg:gap-3'>
			<div className='flex flex-wrap items-center justify-between gap-3 rounded-md border border-gray-200 bg-white p-4 max-sm:p-3'>
				<div className='flex shrink-0 items-center gap-2'>
					...
					<span className='min-w-0 whitespace-nowrap px-1 text-base font-semibold text-gray-900 max-sm:text-sm'>
						{weekRangeLabel}
					</span>
					<Button
						type='button'
				...
				<div className='flex min-w-0 flex-wrap items-center justify-end gap-2 max-sm:ml-auto'>
					...
					/>
					<Button type='button' color='primary' size='sm' onClick={handleShowWeek}>
				...
				<div className='min-h-0 flex-1 overflow-auto '>
					<div className='overflow-x-auto'>
						<div className='flex min-w-[920px] flex-col gap-2'>
							<div className='grid grid-cols-[2.75rem_repeat(7,minmax(0,1fr))] gap-2'>
								<div className='min-h-18 opacity-0' aria-hidden />
								{dayColumns.map(col => {
									...
									return (
										...
										>
											<span className='text-base leading-tight max-sm:text-sm'>
												{intl.formatMessage({ id: `schedule_weekday_${wk}` })}
											</span>
							...
							{lessonNumbers.map(lesson => (
								<div key={lesson} className='grid grid-cols-[2.75rem_repeat(7,minmax(0,1fr))] gap-2'>
									<div className='flex min-h-[104px] items-stretch justify-center py-0.5 max-sm:min-h-[96px]'>
										<div className='flex h-full min-h-22 w-10 shrink-0 items-center justify-center rounded-md bg-accent shadow-sm'>
											<span className='text-xl font-bold tabular-nums text-white max-sm:text-lg'>{lesson}</span>
										</div>
									</div>
									{dayColumns.map(col => {
